    ...
    amp(tr,:) = abs(raw_hilbert(tr,:));
               ...
               % Get Theta Data new way (old way used same start and end inds).
                ...
                if length(spike_peaks_filt)>length(raw_peaks) % Sometimes a bug creeps in, likely
                 ...
                 for s = 1:length(spike_peaks)                               % For each discovered spike
                     ...
                     if spike_peaks(s)>W(end) && spike_peaks(s)<length(ff)+W(1) % Don't include weirdness at the start and end of the filtered trace                        
                        ...
                        if waveform(s,end)>length(ratio) % If the waveform goes over the edge of the ratio vector
                            % Pad with ones
                            new_temp = ratio(waveform(s,1):end,tr)';
                            new_temp2 = ones((waveform(s,end)-length(ratio)),1)'; 
                            if size(new_temp,1)>1
                                new_temp = new_temp';
                            end
                        ...
                        if ratio_waveform(s,1)<1 % If the waveform begins before the ratio vector
                           before_inds = ratio_waveform(s,:)<1;
                           after_inds = ratio_waveform(s,:)>0;
                           
                            new_temp = ones(1,sum(before_inds));
                            new_temp2 = ratio(1:sum(after_inds)); 
                            if size(new_temp,1)>1
                                new_temp = new_temp';
                            end
                            if size(new_temp2,1)>1
                                new_temp2 = new_temp2';
                            end
                        ...
                        if ratio_waveform(s,1)>1 & ratio_waveform(s,end)<=length(ratio(:,tr))
                            ratio_spikes(s,:) = ratio(ratio_waveform(s,:),tr);
                        end
                        if ratio_waveform(s,end)>length(ratio(:,tr))
                            temp_ind = find(ratio_waveform(s,:)==length(ratio(:,tr)));
                            temp_spike = ratio(ratio_waveform(s,1:temp_ind),tr)';
                            temp_spike2 = ones(1,ratio_waveform(s,end)-length(ratio(:,tr)));
                            ratio_spikes(s,:) = [temp_spike,temp_spike2];
                        end
           ...
           if sum(strcmp(varargin{1},'No_Filt'))>0 % If the data and templates are optimally filtered
                ...
                half_height = (AP_height(s,:)+1)/2;
                        ...
                        [~,half_ind] = min(abs(Spike_Stack_original(s,(peak_ind-round(length(Spike_Stack_original(s,:))/4-1)):peak_ind)-half_height)); % Find the point closest to half height.
                        ...
                        [~,half_ind] = min(abs(Spike_Stack_original(s,(peak_ind:peak_ind+round(length(Spike_Stack_original(s,:))/4-1)))-half_height)); % Find the point closest to half height.
