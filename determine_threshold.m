...
% done by shuffling the data and drawing randomly from it.
        ...
        boot_ratio = [];
                   ...
                   % Old way was to shuffle data, but this gives a
                   % conservative estimate because it includes data points
                   % from spikes as well
                   
            % pull_from_data = data(ASAPtime > cutoff & ASAPtime < ASAPtime(end)-cutoff,tr);
            % noise_samples = randsample(ff,20*length(ff),true);
            
            % Generate data from background mean and from Noise_std
            % obtained using the 3-stage procedure in filter_for_detection
            
            noise_samples = normrnd(mean(B_vec),Noise_std,[1 length(ff)]);
            noise_samples = noise_samples';
            
            noise_samples_mean = mean(base(:,tr));
            noise_samples_std = Noise_std; % Uses the 3 step procedure developed in filter_for_detection
            ...
            for t = 1:length(noise_samples)-size(template,2)
                ...
                temp = normcdf(noise_samples(x(dip)), mean(noise_samples), Noise_std); % Chance it was noise
