...
% Bonferroni corrections are also known to err on the side of caution and give false negatives, so it will
...
% 'min_interval' - A time interval (in seconds!) such that if peaks occur within it of
% eachother, the largest peak in the likelihood vector is chosen and the
% others are deleted. Workaround for the same event triggering multiple
% detections if it is oscillating around the threshold. Interval given in
% seconds.
...
% making it much harder to detect spikes. The best approach is to use
% templates taken from the dataset you wish to do detection on. The
% detection is only as good as the templates you choose.

% 4) If you are getting issues, check that the template and the data the
% probabilities are being calculated on are scaled the same.

% 5) This program takes a 3 step approach to estimating the noise in the
% data. The first uses a gaussian mixed model to find the noise
% distribution. If that fails, it takes the side of the distribution where
% the signal isn't (by seeing which way the template goes, up or down), and
% uses that half of the distribution as the estimate of the std of the
% noise. If that fails, it just takes the std of the whole vector as the
% noise estimate. This last option is very conservative, as it
% overestimates noise and will give more false negatives, but it is the
% last approach if the other 2 fail.
...
% it, and will represent the likelihood that your data contains your template.
...
% If you are having issues, try making sure your data is normalized to 1 or
% email me swenceslaoe@hotmail.com.
...
if isempty(ratio_thresh)==1 % If the user wants to determine a threshold by estimating FP rates from the statistics of the data
    [ratio_thresh,FN,TP,FP,mean_boot,std_boot,S_mean,signal_likelihood_std,signal_dist,noise_dist,d_prime,B_mean] = determine_threshold(ff,data,template,template_mean,base,B_vec,updown,Noise_std,signal_std,ASAPtime,cutoff,desired_FP,ratio_thresh,tr);
...
[spike_peaks_filt,raw_peaks,spike_peaks,section,closest_to_start] = find_spikes(ASAPtime,cutoff,ratio,ff,fF,smooth_wave,data,ratio_thresh,updown,pre_spike,post_spike,fs,tr,varargin); % Pull out spike times and waveforms
...
%% Make plot of signal and noise distributions, with threshold, FP and FN rates
     ...
     if isempty(signal_dist)==0 % If we set a threshold. Skipped getting stats because it includes lots of movement, which we have to take out.
        ...
        spikes_struct(1).template = template;
        spikes_struct(1).desired_FP = desired_FP;
        spikes_struct(1).pre_spike = pre_spike;
        spikes_struct(1).post_spike = post_spike;
        spikes_struct(1).cutoff = cutoff;
        spikes_struct(1).fs = fs;
        spikes_struct(1).varargin = varargin; % Save user parameters
